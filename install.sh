...
echo 'alias sudex="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/ziadzoalnoon/core" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/ziadzoalnoon/core'"
...
rm -rf "$HOME/.config/@sudex"
rm -rf "$HOME/.config/@sudex"
rm -rf "$HOME/.config/sudex-core"
